   ...
   internal static class ThreadTest
    {
        ...
        public static void Run()
        {
            ...
            // потоки это очеь затратная для системы функция и поэтому нужно стараться не создавать их часто а использовать текущие 
           ...
           if (! timerThread.Join(300))//таким образом мы заставялем наш главный поток ждать пока не завершиться вторичный , но в таком случаем мы можем получить мертвую блакировку ибо первый поток никогда не закончиться а второй будет его ждать
            {
                ...
                //сверху мы сбросили флаг и таким образом мы просто ждем 300 милисек чтобы наш цикл завршилс и мы смогли выйти из потока 

                 //если у нас не удалось синхронизироваться с укзаныым потоком за указанное время то мы его останавливаем жестком (методом Interrupt)
